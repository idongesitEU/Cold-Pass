...
function createCurve(curveDef, defHash) {
    const create = (hash2) => weierstrass({
        ...curveDef,
        ...getHash(hash2)
    });
    return Object.freeze({
...
// derive-eth-key.js
async function deriveEthPrivateKey(mnemonic, passphrase = "", n2 = 0) {
    if (!Number.isInteger(n2) || n2 < 0) throw new Error("n must be >= 0");
    const words2 = mnemonic.trim().split(/\s+/);
    if (words2.length < 12) throw new Error("Invalid mnemonic");
    const salt = "mnemonic" + passphrase;
    const mnemonicBuffer = toUtf8Bytes(mnemonic.normalize("NFKD"));
    const saltBuffer = toUtf8Bytes(salt.normalize("NFKD"));
    const seed = await pbkdf22(
        mnemonicBuffer,
        saltBuffer,
        2048,
        64,
    ...
    const path = `m/44'/60'/0'/0/${n2}`;
    const child = master.derivePath(path);
    return child.privateKey;
}

// xorTransform.js
function xorTransform(inputStr, base, n2 = 1) {
    if (base < 2 || base > 36)
    ...
    let bits = numBig.toString(2).split("").map((b2) => b2 === "1" ? 1 : 0);
    for (let round = 0; round < n2; round++) {
        const next = [];
    ...
    function bigIntToBase(big, base2) {
        if (big === 0n) return "0";
        const alphabet = "0123456789abcdefghijklmnopqrstuvwxyz".slice(0, base2);
        let result = "";
        let temp = big < 0n ? -big : big;
        while (temp > 0n) {
            result = alphabet[temp % BigInt(base2)] + result;
            temp = temp / BigInt(base2);
        }
        return result;
    }
...
// convertBase.js
function convertBase(inputStr, inputCharset, outputCharset) {
    ...
    function getCharset(baseOrSet) {
        if (typeof baseOrSet === "number") {
            const standard = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
...
function sortByAscii(arr) {
    return arr.slice().sort((a, b2) => compareAscii(String(a), String(b2)));
}
...
window.wordlist = ethers_exports.LangEn.wordlist()._decodeWords();
